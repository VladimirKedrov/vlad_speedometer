local vlad_gears_compat = Config.VladGearsCompatability
--local debug = Config.Debug
local showSpeedo = Config.ShowSpeedometerByDefault
--SETTINGS--
--showFuelGauge = false -- use fuel gauge?
skins = {}
--SETTINGS END--
local overwriteAlpha = true
scriptReady = false
...
function toggleSpeedo()
	if showSpeedo == true then
		showSpeedo = false
	elseif showSpeedo == false then
		showSpeedo = true
	else
		showSpeedo = true
	end
...
Citizen.CreateThread(function()
	...
	while true do
		Citizen.Wait(0)
		if (inVehicleAtGetin or inVehicle) then
			if not showSpeedo then curAlpha = 0 end
			if showSpeedo then
